...
func InitGateioClient(cfg *commonlib.HttpClientConf) {
	if uriList == nil || len(uriList) == 0 {
		uriList = make(map[string]string)
		uriList["GetAllCoins"] = "/api/v4/spot/currencies"
	}
	httpClient = commonlib.GetDefaultHttpClient(cfg.TimeoutSec)
	fmt.Println("InitGateioClient done")
}

func getEndPoint(uri string) string {
	uri, ok := uriList[uri]
	if ok && len(uri) > 0 {
		return commonlib.LaunchConfig().Gateio.Host[0] + uri
	}
	return ""
}
