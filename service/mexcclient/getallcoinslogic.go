...
func GetAllCoins(ctx context.Context) (map[string]*CoinItemVo, error) {
	...
	dlog.Infof("%v||mexcclient->GetAllCoins done,err=%v", trc, err)
	if err != nil || len(resp) == 0 {
		return nil, fmt.Errorf("mexcclient->GetAllCoins fail, err=%v or resp=nil", err)
	}

	var r GetAllCoinsResp
	err = json.Unmarshal(resp, &r)
	if err != nil || len(r.Symbols) == 0 {
		return nil, fmt.Errorf("mexcclient->GetAllCoins Unmarshal fail or no data, err=%v", err)
	}
