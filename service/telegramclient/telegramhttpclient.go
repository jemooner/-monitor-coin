...
func InitTelegramClient(cfg *commonlib.HttpClientConf) {
	if uriList == nil || len(uriList) == 0 {
		uriList = make(map[string]string)
		uriList["SendMessage"] = "/bot5704936339:AAE25NY-PHY2ESyNroxf9XlrJtvZXsQ98WA/sendMessage"
	}
	httpClient = commonlib.GetDefaultHttpClient(cfg.TimeoutSec)
	fmt.Println("InitTelegramClient done")
}

func getEndPoint(uri string) string {
	uri, ok := uriList[uri]
	if ok && len(uri) > 0 {
		return commonlib.LaunchConfig().TeleGram.Host[0] + uri
	}
	return ""
}

func doHttpRequest(ctx context.Context, m string, ep string, ps map[string]interface{}, chatId string, message string) (r []byte, err error) {
	ts, traceId := time.Now(), commonlib.GetTrace(ctx)
	dlog.Infof(commonlib.CallInLog(ctx, "telegram", m, ep, ps))
	...
	dlog.Infof(commonlib.CallOutLog(ctx, "binance", []byte(""), err, ts))
