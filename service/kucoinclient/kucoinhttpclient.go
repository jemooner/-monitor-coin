...
func InitKucoinClient(cfg *commonlib.HttpClientConf) {
	if uriList == nil || len(uriList) == 0 {
		uriList = make(map[string]string)
		uriList["GetAllCoins"] = "/api/v1/symbols"
	}

	httpClient = commonlib.GetDefaultHttpClient(cfg.TimeoutSec)
	fmt.Println("InitKucoinClient done")
}

func getEndPoint(uri string) string {
	uri, ok := uriList[uri]
	if ok && len(uri) > 0 {
		return commonlib.LaunchConfig().Kucoin.Host[0] + uri
	}
	return ""
}
