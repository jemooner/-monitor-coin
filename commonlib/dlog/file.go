...
func (self *FileBackend) rotateByHourDaemon() {
	...
	for {
		...
		if self.lastCheck < check {
			for i := 0; i < numSeverity; i++ {
				fi, fiErr = os.Stat(self.files[i].filePath)
		...
		for _, file := range files {
			...
			if file.Name() == self.reg.FindString(file.Name()) && shouldDel(file.Name(), self.keepHours) {
...
func NewFileBackend(logConfig LogConfig) (*FileBackend, error) {
	if err := os.MkdirAll(logConfig.FilePath, 0755); err != nil {
		return nil, err
	}
	var fb FileBackend
	fb.dir = logConfig.FilePath
	for i := 0; i < numSeverity; i++ {
		fileName := path.Join(logConfig.FilePath, severityName[i]+".log")
	...
	logConfig.FileRotateCount = 20
	logConfig.FileRotateSize = 1024 * 1024 * 1024
	fb.Rotate(logConfig.FileRotateCount, logConfig.FileRotateSize)
	fb.SetFlushDuration(3 * time.Second)
	fb.SetRotateByHour(logConfig.RotateByHour)
	fb.SetKeepHours(logConfig.KeepHours)
